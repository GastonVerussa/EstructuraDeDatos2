...
public class GrafoEtiquetadoViejo {
    ...
    public boolean eliminarVertice(Object vertice){
        ...
        if(!this.esVacio()){
            ...
            if(inicio.getElem().equals(vertice)){
                //  Primero eliminamos todos los arcos que tienen como destino el vertice a eliminar
                NodoAdy adyacenteOrigen = inicio.getPrimerAdy();
                //  Para eso revisamos todos los arcos que este vertice tenga de origen
                while(adyacenteOrigen != null){
                    //  Consigue el vertice destino del arco (Hay que eliminar en ambos sentidos, ya que es grafo y no digrafo)
                    NodoVert auxiliar = adyacenteOrigen.getVertice();
                    //  Auxiliar para recorrer los adyacentes del vertice destino
                    NodoAdy AdyAuxiliar = auxiliar.getPrimerAdy();
                    //  Si el primer arco es el correcto
                    if(AdyAuxiliar.getVertice().equals(inicio)){
                        //  Se elimina seteando un nuevo primer Adyacente
                        auxiliar.setPrimerAdy(AdyAuxiliar.getSigAdyacente());
                    } else {
                        //  Si no, se busca el arco correcto, siempre revisando desde el anterior
                        while(!AdyAuxiliar.getSigAdyacente().getVertice().equals(inicio)){
                            AdyAuxiliar = AdyAuxiliar.getSigAdyacente();
                        }
                        //  Cuando se lo encuentra, se borra simplemente de la lista de los adyacentes
                        AdyAuxiliar.setSigAdyacente(AdyAuxiliar.getSigAdyacente().getSigAdyacente());
                    }
                    //  Una vez que ya se borro el arco del otro lado, se va al siguiente adyacente 
                    adyacenteOrigen = adyacenteOrigen.getSigAdyacente();
                }
                ...
                while(aux != null && !exito){
                    ...
                    if(aux.getElem().equals(vertice)){
                        //  Primero eliminamos todos los arcos que tienen como destino el vertice a eliminar
                        NodoAdy adyacenteOrigen = aux.getPrimerAdy();
                        //  Para eso revisamos todos los arcos que este vertice tenga de origen
                        while(adyacenteOrigen != null){
                            //  Consigue el vertice destino del arco (Hay que eliminar en ambos sentidos, ya que es grafo y no digrafo)
                            NodoVert auxiliar = adyacenteOrigen.getVertice();
                            ...
                            NodoAdy AdyAuxiliar = auxiliar.getPrimerAdy();
                            //  Si el primer arco es el correcto
                            if(AdyAuxiliar.getVertice().equals(aux)){
                                //  Se elimina seteando un nuevo primer Adyacente
                                auxiliar.setPrimerAdy(AdyAuxiliar.getSigAdyacente());
                            } else {
                                //  Si no, se busca el arco correcto, siempre revisando desde el anterior
                                while(!AdyAuxiliar.getSigAdyacente().getVertice().equals(aux)){
                                    AdyAuxiliar = AdyAuxiliar.getSigAdyacente();
                                }
                                //  Cuando se lo encuentra, se borra simplemente de la lista de los adyacentes
                                AdyAuxiliar.setSigAdyacente(AdyAuxiliar.getSigAdyacente().getSigAdyacente());
    ...
    @Override
    public GrafoEtiquetadoViejo clone(){
        
        //  Nuevo grafo que se pasara como resultado final
        GrafoEtiquetadoViejo resultado = new GrafoEtiquetadoViejo();
